...
max_horse_pop = max(horse_pop$Value)
```

From the above, we also get the maximum horse population value as `r max_horse_pop`.
...
```{r plot horses, fig.width=10, fig.height=3, fig.cap="Historical number of horses per province in Canada"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Table 1. Car sales per year for each province between 1940 - 1972")
```

Below we zoom in and look at the province of Alberta:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Historical number of horses per province in Canada"}
